<template>
  <div>
    <h1>月營收總結</h1>
    <div v-if="loading">
      <p>loading...</p>
    </div>
    <div v-else>
      <div>
        <label>
          公司編號：
          <input type="text" v-model="filterCompany" />
        </label>
        <label>
          年月：
          <input type="text" v-model="filterYYmm" />
        </label>
        <button @click="fetchData"style="margin-bottom: 20px;">查詢</button>
        <button @click="navigateToAddRevenueItem" style="margin-bottom: 20px;">新增營收項目</button>
      </div>
      <div v-if="totalRecords > 0">
        <table>
          <thead>
            <tr>
              <th>產生報告日期</th><th>公司編號</th>
              <th>公司名稱</th><th>行業</th>
              <th>本月營收</th><th>上個月營收</th><th>去年同月營收</th>
              <th>月營收增減(%)</th><th>去年同月增減(%)</th>
              <th>本累計營收</th><th>去年累計營收</th>
              <th>累計增減(%)</th><th>備註</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(record, index) in Data" :key="index">
              <td>{{ record.reportDate }}</td><td>{{ record.companyCode }}</td>
              <td>{{ record.companyName }}</td><td>{{ record.industry }}</td>
              <td>{{ record.nowRevenue }}</td><td>{{ record.lastMonthRevenue }}</td>
              <td>{{ record.lastYearSameMonthRevenue }}</td>
              <td>{{ getFormattedChange(record.monthlyPercent) }}</td><td>{{ getFormattedChange(record.yearlyPercent) }}</td>
              <td>{{ record.cumulativeNowRevenue }}</td><td>{{ record.cumulativeLastYearRevenue }}</td>
              <td>{{ getFormattedChange(record.cumulativeChangePercent) }}</td><td>{{ record.comment }}</td>
            </tr>
          </tbody>
        </table>
        <p>總筆數：{{ totalRecords }}</p>
        <div class="pagination">
          <button :disabled="currentPage === 1" @click="changePage(currentPage - 1)">上一頁</button>
          <span>第 {{ currentPage }} 頁，共 {{ totalPages }} 頁</span>
          <button :disabled="currentPage === totalPages" @click="changePage(currentPage + 1)">下一頁</button>
        </div>
      </div>
      <div v-else>
        <p>無條件符合</p>
      </div>
    </div>
  </div>
</template>

<script>
import * as api from '../services/api';

export default {
  data() {
    return {
      Data: [],
      filterCompany: '',
      filterYYmm: '',
      totalRecords: 0,
      loading: true,
      currentPage: 1,
      pageSize: 10,
      totalPages: 0 
    };
  },
  mounted() {
    this.fetchData();
  },
  methods: {
    async fetchData() {
      try {
        const response = await api.getMonthlySummary({
          dataYearMonth: this.filterYYmm,
          companyCode: this.filterCompany,
          pageNumber: this.currentPage,
          pageSize: this.pageSize
        });
        this.Data = response.items; 
        this.totalRecords = response.totalRecords; 
        this.totalPages = Math.ceil(this.totalRecords / this.pageSize); 
        this.loading = false;
      } catch (error) {
        console.error('操作錯誤:', error);
        this.loading = false;
      }
    },
    changePage(page) {
      if (page >= 1 && page <= this.totalPages) {
        this.currentPage = page;
        this.fetchData();
      }
    },
    getFormattedChange(changePercent) {
      return changePercent != null ? changePercent.toFixed(2) : '無';
    },
    navigateToAddRevenueItem() {
      this.$router.push({ name: 'AddItem' });
    }
  }
};
</script>

<style scoped>
h1 {
  font-size: 1.5em;
  margin-bottom: 20px;
}
table {
  width: 100%;
  border-collapse: collapse;
  margin-top: 20px;
}
th, td {
  padding: 12px;
  border: 1px solid #ddd;
  text-align: left;
}
th {
  background-color: #f4f4f4;
}
tr:nth-child(even) {
  background-color: #f9f9f9;
}
input[type="text"] {
  margin: 10px;
  padding: 5px;
  font-size: 1em;
}
.pagination {
  margin-top: 20px;
  display: flex;
  justify-content: center;
  align-items: center;
}
button {
  margin: 0 10px;
  padding: 10px;
  font-size: 1em;
}
</style>
